...
document.localization ['ja_JP'] = {
    ...
    'form.configuration.settings.push.options.mqtt.alarm': 'MQTTプロトコル(即時配信)',
    ...
    'form.configuration.settings.push.options.mqtt.alarm.hint' : 'プッシュ メッセージを即座に配信しますが、デバイスがスリープ状態のときにより多くの電力を消費する場合があります。 スタンドアロンの受電デバイスに最適なオプションです。 MQTT は TCP ポート 31000 を使用します。ファイアウォールで有効にしてください。',
    'form.configuration.settings.push.options.polling.hint': '最大 15 分の遅延でプッシュ メッセージを確実に配信します。 HTTP(S) 以外の TCP ポートは必要ありません。 このオプションをリモート コントロール プラグインと一緒に使用しないでください。',
    ...
    'form.configuration.settings.use.default.launcher': 'デフォルトのランチャーで使用',
